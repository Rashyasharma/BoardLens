...
// filtering for results where PUM is 0 AND the grade is not X or Q (legitimate grades with 0 PUM).
...
echo "=== SERIES & QUALIFICATIONS WITH MISSING PUM VALUES (EXCLUDING X/Q/U/PENDING/ENTRY) ===" . PHP_EOL;
...
foreach ($seriesList as $series) {
    foreach ($qualifications as $qual) {
        ...
        // Filter: PUM is 0.0 AND the grade is legitimate (not U, X, Q, ENTRY, PENDING)
        $zeroPum = SubjectResult::where('series_id', $series->id)
            ->where('pum', 0.0)
            ->whereNotIn('grade', ['U', 'X', 'Q', 'ENTRY', 'PENDING', 'ungraded', 'UNGRADED'])
            ->whereHas('subject', function($q) use ($qual) {
                $q->where('qualification_id', $qual->id);
            })->count();

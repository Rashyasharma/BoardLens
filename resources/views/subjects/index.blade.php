        ...
        @foreach($qual['subjects_with_stats'] as $subj)
            @php
                $allSubjects[] = [
                    ...
                    'missing_current_year_set' => $subj['missing_current_year_set'] ?? false
                ];
            @endphp
        @endforeach
    @endforeach
            ...
            >
                <div class="space-y-4">
                    ...
                    <div class="flex items-center justify-between">
                        <span class="font-mono text-[10px] font-bold {{ $color['code'] }} px-2 py-0.5 rounded-md border border-black/5 shadow-xxs flex items-center gap-1">
                            #{{ $s['code'] }}
                            @if($s['missing_current_year_set'])
                                <svg class="w-3.5 h-3.5 text-rose-500 animate-bounce" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" title="Missing Component Set for Current Year">

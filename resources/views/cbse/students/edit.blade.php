...
@section('title', 'Edit CBSE Student')
@section('page-title', 'Edit CBSE Student')
...
<div class="space-y-6 max-w-2xl">
    ...
    <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        <form method="POST" action="{{ route('cbse.students.update', $student->id) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <div class="space-y-1.5">
                <label for="admission_number" class="text-xxs font-bold text-slate-400 uppercase tracking-wider">Admission Number</label>
                <input type="text" name="admission_number" id="admission_number" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" required value="{{ $student->admission_number }}">
            </div>

            <div class="space-y-1.5">
                <label for="student_name" class="text-xxs font-bold text-slate-400 uppercase tracking-wider">Student Name</label>
                <input type="text" name="student_name" id="student_name" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" required value="{{ $student->student_name }}">
            </div>

            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    <label for="father_name" class="text-xxs font-bold text-slate-400 uppercase tracking-wider">Father's Name</label>
                    <input type="text" name="father_name" id="father_name" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" value="{{ $student->father_name }}">
                </div>

                <div class="space-y-1.5">
                    <label for="mother_name" class="text-xxs font-bold text-slate-400 uppercase tracking-wider">Mother's Name</label>
                    <input type="text" name="mother_name" id="mother_name" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" value="{{ $student->mother_name }}">
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="date" name="date_of_birth" id="date_of_birth" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" value="{{ $student->date_of_birth ? $student->date_of_birth->format('Y-m-d') : '' }}">
                </div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <select name="qualification_type" id="qualification_type" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" required>
                        <option value="CLASS_10" {{ $student->qualification_type == 'CLASS_10' ? 'selected' : '' }}>Class 10 (Secondary)</option>
                        <option value="CLASS_12" {{ $student->qualification_type == 'CLASS_12' ? 'selected' : '' }}>Class 12 (Senior Secondary)</option>
                    </select>
                </div>

                <div class="space-y-1.5">
                    <label for="admission_year" class="text-xxs font-bold text-slate-400 uppercase tracking-wider">Admission Year</label>
                    <input type="number" name="admission_year" id="admission_year" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" required value="{{ $student->admission_year }}" min="2000" max="2050">
                </div>
            </div>
            ...
            <div class="space-y-1.5">
                ...
                <select name="status" id="status" class="w-full bg-slate-50 border border-slate-250 rounded-xl px-4.5 py-2.5 text-sm font-semibold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:bg-white transition-all duration-150" required>
                    ...
                    <option value="passed" {{ $student->status == 'passed' ? 'selected' : '' }}>Passed Out</option>

...
@section('title', 'Create Exam Series')
@section('page-title', 'Create Exam Series')
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-2 text-xs text-slate-400 font-semibold">
        ...
        <span>›</span>
        <span class="text-slate-600">Create New Series</span>
    </div>
    ...
    <form action="{{ route('exam-series.store') }}" method="POST" class="space-y-6">
        @csrf

        {{-- Year --}}
        <div class="bg-white rounded-2xl border border-slate-150 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                <h2 class="text-sm font-bold text-slate-700 uppercase tracking-wider">Step 1 — Year</h2>
            </div>
            <div class="px-6 py-5">
        ...
        <div class="bg-white rounded-2xl border border-slate-150 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                <h2 class="text-sm font-bold text-slate-700 uppercase tracking-wider">Step 2 — Exam Session</h2>
            </div>
            <div class="px-6 py-5">
                <label class="block text-xs font-semibold text-slate-500 uppercase mb-3">Select Session</label>
        ...
        <div class="bg-white rounded-2xl border border-slate-150 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                <h2 class="text-sm font-bold text-slate-700 uppercase tracking-wider">Optional — Dates & Status</h2>
            </div>
            <div class="px-6 py-5 grid grid-cols-1 sm:grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-slate-500 uppercase mb-2">Entry Deadline</label>
                    <input type="date" name="deadline_for_entry" value="{{ old('deadline_for_entry') }}"
                        class="w-full px-3 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500/20" />
                </div>
                <div>
                ...
                <div class="sm:col-span-2">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <span class="text-sm font-semibold text-slate-700">Mark this series as <span class="text-indigo-600">Active</span></span>
                    </label>
                </div>
            </div>
        </div>

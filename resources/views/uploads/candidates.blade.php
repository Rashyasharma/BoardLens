...
@section('title', 'Upload Candidate Profiles - Cambridge Exam Portal')
@section('page-title', 'Upload Candidate Profiles')
...
<div class="grid grid-cols-12 gap-8">
    ...
    <div class="col-span-12 lg:col-span-4 space-y-6">
        <div class="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm">
            <h3 class="text-lg font-bold text-slate-800 tracking-tight">Upload Candidates Spreadsheet</h3>
            <p class="text-sm text-slate-500 mt-1">Bulk upload student/candidate profiles to the database.</p>

            <form method="POST" action="{{ route('uploads.candidates.store') }}" enctype="multipart/form-data" class="space-y-5 mt-6">
                @csrf
                
                <!-- File Selector -->
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1.5">Spreadsheet File (CSV / XLSX)</label>
                    <input type="file" name="candidate_file" required accept=".csv,.xlsx,.xls"
                ...
                <table class="w-full text-left">
                    ...
                    <tbody class="divide-y divide-slate-900/60">
                        <tr>
                            ...
                            <td class="text-emerald-400">CN006</td>
                        </tr>
                        <tr>
                            <td class="py-1">B</td>
                            <td class="text-slate-300">candidate_name</td>
                            <td class="text-emerald-400">David Miller</td>
                        </tr>
                        <tr>
                            <td class="py-1">C</td>
                            <td class="text-slate-300">date_of_birth</td>
                            <td class="text-emerald-400">2010-08-14</td>
                        </tr>
                        <tr>
                            <td class="py-1">D</td>
                            <td class="text-slate-300">gender</td>
                            <td class="text-emerald-400">M</td>
                        </tr>
                    </tbody>
                </table>
            </div>
    ...
    <div class="col-span-12 lg:col-span-8 bg-white border border-slate-200 rounded-3xl shadow-sm overflow-hidden flex flex-col justify-between">
        <div>
            <div class="px-6 py-5 border-b border-slate-200 bg-slate-50/50">
                <h3 class="text-lg font-bold text-slate-800">Recent Candidates Uploads</h3>
                <p class="text-sm text-slate-500 mt-0.5">Logs of recently imported candidate profile spreadsheets</p>
            </div>
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="py-12 px-6 text-center text-slate-400">
                                    No candidate uploads logged yet.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
